...
def main():
    ...
    N = 12 # Number of shell candidates to plot
    ...
    for n in range(N):
        ...
        pv = plot_pv(cube=cube_file, ra_center=shell.ra, dec_center=shell.dec,
            vel=[shell.vmin, shell.vmax], length=shell.radius*2.,
            width=7.5*u.arcsec, angle=0*u.deg,
            pad_factor=1., plotname='13co_pv_shell'+str(n+1)+'_angle0.png', return_subplot=True,
...
def simbad_sources(ra, dec, radius, unit='degree', object_type="Star"):
    ...
    result_table = Simbad.query_criteria("region(circle, " + str(c.ra.to(u.deg).value) + " "
...
def plot_overview(cube='../nro_maps/12CO_20161002_FOREST-BEARS_spheroidal_xyb_grid7.5_0.099kms.fits',
 ...
 interactive=False, show_shells=True):
    """
